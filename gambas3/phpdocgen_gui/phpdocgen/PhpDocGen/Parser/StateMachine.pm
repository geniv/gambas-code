...
PhpDocGen::Parser::StateMachine - A state machine
...
PhpDocGen::Parser::StateMachine is a Perl module, which 
...
package PhpDocGen::Parser::StateMachine;
...
sub changestatefrom($)  {
  ...
  if ( ! isemptyhash( $trans ) ) {
   ...
   if ( defined $for_event ) {
    ...
    confess( "Unable to find a transition from ".
    	     "the state '".
...
sub changestateforEOF()  {
  ...
  if ( ! isemptyhash( $trans ) ) {
    ...
    if ( PhpDocGen::General::Verbose::currentlevel() >= 4 ) {
      my $verbstr = join( '',
     		     	  "\t\ttoken = '",
			  tohumanreadable($self->{'SM_PREVIOUS_TOKEN'}),
			  "'" ) ;
      $verbstr .= ", when EOF in '".$self->{'SM_CURRENT_STATE'}."'" ;
      PhpDocGen::General::Verbose::verb( "$verbstr.", 4 ) ;
    }
...
sub _callback($$$)  {
  my $self = shift ;
  my $token = $_[0] || '' ;
  my $oldstate = $_[1] || confess( 'empty old state' ) ;
  if ( PhpDocGen::General::Verbose::currentlevel() >= 4 ) {
    my $verbstr = join( '',
     		     	"\t\ttoken = '",
  ...
  if ( ishash( $_[2] ) ) {
    if ( exists $_[2]{'callback'} ) {
      ...
      $reffunc = $self->can( "state<_callback_".
      	 	    	     $self->{'SM_CURRENT_STATE'} ) ;
      if ( $reffunc ) {
        $self->$reffunc( $token ) ;
      }
...
sub _searchtransition($$)  {
  ...
  if ( ( $recognizedtoken ) && ( PhpDocGen::General::Verbose::currentlevel() >= 5 ) ) {
    PhpDocGen::General::Verbose::verb( join( '',
					     "\t\tnext transition:\n\t\t\tfrom: ",
					     $self->{'SM_CURRENT_STATE'},
					     "\n\t\t\tto: ",
					     $trans->{'state'},
